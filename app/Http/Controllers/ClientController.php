...
class ClientController extends Controller {
    ...
    public function invoice_payment($invoice_id, $payment_method) {
        ...
        if ($payment_method == 'paypal') {
            ...
            $session = \Stripe\Checkout\Session::create([
                ...
                'line_items'           => [[
                    'price_data' => [
                        ...
                        'unit_amount'  => round(convert_currency(get_company_field($data['invoice']->company_id, 'base_currency', 'USD'), get_company_field($data['invoice']->company_id, 'stripe_currency', 'USD'), $amount)) * 100,
                ...
                'cancel_url'           => url('client/stripe_payment/cancel/' . $invoice_id),
            ]);
            ...
            $orderData = [
                ...
                'amount'          => round(convert_currency(get_company_field($data['invoice']->company_id, 'base_currency', 'USD'), 'INR', $amount)) * 100,
                'currency'        => 'INR',
    ...
    public function stripe_payment($action, $invoice_id) {
        ...
        if ($action == 'cancel') {
            return redirect('client/view_invoice/' . $invoice_id)->with('error', _lang('Payment Cancelled !'));
        }
